...
const ItemsScrollableItem = styled('div')`
  ...
  &:last-of-type {
    flex: 0 0 calc(28rem + ${(props) => props.theme.layout.padding});
    padding: 0 ${(props) => props.theme.layout.padding} 0 0;
    @media (max-width: ${(props) => props.theme.breakpoints.tablet}) {
      flex: 0 0 calc(28rem + ${(props) => props.theme.layout.paddingMobile});
      padding: 0 ${(props) => props.theme.layout.paddingMobile} 0 0;
    }
  }
`

const ItemsScrollable = ({ items, renderItem, children }) => {
  ...
  return (
    <ItemsScrollableView>
      {hasItems
        ? items.map((item) => (
            <ItemsScrollableItem key={item.key}>
              {renderItem({ item })}
            </ItemsScrollableItem>
          ))
        : children}
